...
def index():
    ...
    transactions_db = db.execute("SELECT symbol, SUM(shares) AS totalshare, price, (SUM(shares) * price) AS value FROM transactions WHERE user_id = ? GROUP BY symbol", user_id)
    cash_db = db.execute("SELECT cash FROM users WHERE id = ?", user_id)

    cash = cash_db[0]["cash"]
    total = cash
    for row in transactions_db:
        total += row["price"] * row["totalshare"]
...
def buy():
    ...
    if request.method == "GET":
        ...
        if not shares:
            return apology("How many shares you want to buy?")

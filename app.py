...
def index():
    ...
    cash = cash_db[0]["cash"]
    money_db = db.execute("SELECT SUM(price * shares) AS money FROM transactions WHERE user_id = ?", user_id)
    money = money_db[0]["money"]

    return render_template("index.html", database = transactions_db, cash = cash, money = money)
